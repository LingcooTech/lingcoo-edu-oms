...
- `application_branding` 保存界面展示名称、五个受控颜色 Token、标题/正文字体、0–24 px 圆角、登录页标题/副标题和 Revision；
- 完整 Logo、方形 Logo、暗色 Logo 与 Favicon 只保存在 `storage_asset_references`，分别使用
  `application-branding/default/logo`、`square-logo`、`dark-logo` 和 `favicon`；
...
| GET    | `/api/branding/assets/square-logo` | 显式公开          |
| GET    | `/api/branding/assets/dark-logo`   | 显式公开          |
| GET    | `/api/branding/assets/favicon`     | 显式公开          |

公开配置不返回 Asset ID、操作人或 Storage Provider 信息。响应使用内容摘要 ETag 和强制重新验证缓存；品牌素材
...
`/admin/branding` 按“品牌标识、品牌素材、视觉主题、登录界面”组织整体表单和实时预览：

- `branding.read` 控制页面和导航可见性；
- `branding.manage` 控制保存；
- 选择 Asset 还需要 `storage.read`，Starter 不额外复制一套受限素材查询 API；
- 四类品牌素材通过 `AssetPicker` 只提交稳定 Asset ID，后端重新验证 active 与 image 类型；
- 保存成功后同步刷新 Query Cache、Ant Design 品牌色、页面/卡片/文字颜色、字体、圆角、页面标题、Favicon、登录页和 Admin Shell；
- Branding 请求失败时使用本地默认值，不阻断登录或 Admin 启动。

主题模式与品牌色是两个独立概念：用户 light/dark 偏好选择 Ant Design algorithm；深色模式只继承品牌色、字体和圆角，
不强行套用浅色背景/卡片/文字色。服务端不会接受任意 Token Map、CSS、HTML、Markdown 或布局 JSON。

## 安全边界

- 文本均为单行纯文本，拒绝控制字符和 `<` / `>`，React 仍按普通文本渲染；
- 五个颜色 Token 只接受六位 Hex，不接受 `rgb()`、`var()`、渐变或 URL；
- 字体栈限制长度并拒绝 CSS 分隔符、花括号、HTML 和换行；圆角只接受 0–24 的整数；
