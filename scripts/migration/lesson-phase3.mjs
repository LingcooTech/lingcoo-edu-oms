...
import {
  ...
  readLessonSource,
  readLessonTargetReferences,
  targetLessonCollisions,
  targetLessonSchemaBlockers,
} from './lesson-phase3-db.mjs';
import {
  LESSON_TOOL_VERSION,
  planLessonMigration,
  publicLessonReport,
} from './lesson-phase3-lib.mjs';
...
async function main() {
  ...
  const source = createDatabaseClient(
    sourceUrl,
    `lingcoo-lesson-phase3-source-${LESSON_TOOL_VERSION}`,
  );
  const target = createDatabaseClient(
    targetUrl,
    `lingcoo-lesson-phase3-target-${LESSON_TOOL_VERSION}`,
  );
  try {
    ...
    const [sourceData, schemaBlockers, references] = await Promise.all([
      readLessonSource(source),
      targetLessonSchemaBlockers(target),
      readLessonTargetReferences(target),
    ]);
    const plan = planLessonMigration(sourceData, references);
    plan.blockers.push(...schemaBlockers);
    if (!schemaBlockers.length) plan.blockers.push(...(await targetLessonCollisions(target)));
    plan.readyToApply = plan.blockers.length === 0;
    const report = publicLessonReport(plan, {
    ...
    if (plan.incomplete.length && !options.allowIncomplete) {
      throw new Error(
        'Plan contains deferred or unresolved records; review and pass --allow-incomplete to acknowledge',
      );
    }
    if (options.confirmTarget !== targetFingerprint) {
      throw new Error(`Target confirmation mismatch; expected ${targetFingerprint}`);
    }
    await applyLessonPlan(target, plan.actions);
    await emit({ ...report, applied: true }, options.reportPath);
  } finally {
    try {
      await source.query('ROLLBACK');
    } catch {
      // Source may not have connected or the read-only transaction may already be closed.
    }
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : 'Lesson phase 3 migration failed');
  process.exitCode = 1;
});
